...
function getTemplates() {
  try { return JSON.parse(localStorage.getItem('tb_templates') || '[]'); } catch { return []; }
}
function setTemplates(list) { localStorage.setItem('tb_templates', JSON.stringify(list)); }
...
function duplicateTemplate(id) {
  ...
  list.push({...src, id:'tpl_'+Date.now(), name:(src.name||'템플릿')+' 복사본', createdAt:now, updatedAt:now});
  setTemplates(list);
  renderTemplateList();
  showToast('복사되었어요');
}
...
function saveTemplate() {
  ...
  const now = new Date().toISOString();
  if(currentTplId) {
    const idx = list.findIndex(t => t.id === currentTplId);
    if(idx >= 0) list[idx] = {...list[idx], name, blocks, nextId, html, updatedAt: now};
    else { list.push({id:currentTplId, name, blocks, nextId, html, createdAt:now, updatedAt:now}); }
  } else {
    currentTplId = 'tpl_' + Date.now();
    list.push({id:currentTplId, name, blocks, nextId, html, createdAt:now, updatedAt:now});
  }
  setTemplates(list);
  showToast('💾 저장되었어요');
}
